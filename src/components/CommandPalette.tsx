...
const CommandPalette = (props: CommandPaletteProps): JSX.Element => {
      ...
      <div class="fixed inset-0 z-50 flex items-start justify-center bg-black/60 px-4 py-24 backdrop-blur-sm">
        <div class="w-full max-w-xl rounded-2xl border border-border/70 bg-surfaceElevated/95 shadow-2xl shadow-accent/20">
          <div class="border-b border-border/60 px-4 py-3">
            <input
              class="w-full rounded-lg border border-border/70 bg-surface px-3 py-2 text-sm text-foreground outline-none transition focus:border-accent/60"
              autofocus
          ...
          <div class="max-h-72 overflow-y-auto px-4 py-3">
            <Show when={commands().length > 0} fallback={<EmptyState />}>
              <For each={commands()}>
                {(command) => (
                  <div class="flex items-center justify-between border-b border-border/20 py-2 text-sm text-muted last:border-none">
                    <div>
          ...
          <div class="flex items-center justify-between border-t border-border/60 px-4 py-3 text-xs text-muted">
            ...
            <button
              class="rounded-lg border border-border/60 bg-surface px-3 py-1 transition hover:border-accent/60 hover:text-accent-foreground"
