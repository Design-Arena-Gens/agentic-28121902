...
const Sidebar = (props) => {
    ...
    const [newLanguage, setNewLanguage] = createSignal("tsx");
    ...
    return (<aside class={`flex shrink-0 flex-col border-r border-border/70 bg-surfaceElevated/60 transition-all duration-300 ${collapsed() ? "w-16" : "w-72"}`}>
      <div class="flex items-center justify-between border-b border-border/60 px-4 py-3">
        ...
        <button class="rounded-lg border border-border/70 bg-surface px-2 py-1 text-xs text-muted transition hover:border-accent/70 hover:text-accent-foreground" onClick={() => setCollapsed((prev) => !prev)}>
          {collapsed() ? "⟨" : "⟩"}
        </button>
      </div>
      <Show when={!collapsed()} fallback={<div class="flex flex-1 flex-col items-center gap-4 px-1 py-4 text-[10px] text-muted">
        ...
        <div class="flex-1 overflow-y-auto px-4 py-3">
          ...
          <section class="mt-4">
            ...
            <div class="mt-2 space-y-1">
              <For each={props.files}>
                {(file) => (<button class={`group flex w-full items-center gap-3 rounded-lg border border-transparent px-3 py-2 text-left text-sm transition hover:border-border/70 hover:bg-surfaceElevated ${file.id === props.activeFileId ? "border-accent/40 bg-surfaceElevated" : ""}`} onClick={() => props.setActiveFile(file.id)}>
          ...
          <form class="mt-4 space-y-2 rounded-xl border border-border/60 bg-surface/70 p-3" onSubmit={createFile}>
            ...
            <input class="w-full rounded-lg border border-border/60 bg-surfaceElevated px-3 py-2 text-sm text-foreground outline-none transition focus:border-accent/70" placeholder="src/components/Preview.tsx" value={newFileName()} onInput={(event) => setNewFileName(event.currentTarget.value)}/>
            <select class="w-full rounded-lg border border-border/60 bg-surfaceElevated px-3 py-2 text-sm text-foreground outline-none transition focus:border-accent/70" value={newLanguage()} onInput={(event) => setNewLanguage(event.currentTarget.value)}>
        ...
        <div class="border-t border-border/60 px-4 py-3">
          ...
          <ul class="mt-2 space-y-1 text-xs text-muted">
            <For each={props.shortcuts}>
              {(shortcut) => (<li class="flex items-center justify-between rounded-lg border border-border/50 bg-surface/60 px-3 py-2">
