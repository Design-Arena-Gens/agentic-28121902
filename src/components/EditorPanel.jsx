...
const EditorPanel = (props) => {
    ...
    return (<section class="flex min-w-[420px] max-w-xl flex-1 flex-col gap-4 overflow-hidden rounded-2xl border border-border/70 bg-surface/80 p-5 shadow-glow">
      <header class="flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <button class="rounded-lg border border-border/60 bg-surface px-3 py-2 text-xs text-muted transition hover:border-accent/60 hover:text-accent-foreground" onClick={() => props.onCommandToggle(true)}>
            Command palette
          </button>
          <button class="rounded-lg border border-border/60 bg-surface px-3 py-2 text-xs text-muted transition hover:border-accent/60 hover:text-accent-foreground" onClick={() => {
            if (!props.file)
                return;
      ...
      <div class="grid gap-3 md:grid-cols-2">
        <SummaryCard title="Claude Alignment" description="Diffs prepared for review. Use ⌘+Enter to accept or request revisions." badge="Ready"/>
        <SummaryCard title="Playground Preview" description="Run `npm run dev` or trigger sandbox preview to validate updates." badge="Next"/>
      </div>
    </section>);
};
const SummaryCard = (props) => {
    const id = createUniqueId();
    return (<div class="rounded-xl border border-border/60 bg-surface px-4 py-3 text-sm text-muted">
...
const EmptyState = () => (<div class="flex h-full flex-col items-center justify-center gap-3 text-center text-sm text-muted">
