...
const TopBar = (props) => {
    return (<header class="flex h-16 items-center justify-between gap-4 border-b border-border/60 bg-surfaceElevated/70 px-6 backdrop-blur-lg">
      ...
      <div class="flex flex-1 items-center justify-center gap-4">
        <div class="flex items-center gap-3 rounded-xl border border-border/80 bg-surface px-3 py-2 shadow-inner shadow-black/40">
          <Show when={props.assistants.length > 0}>
            <select class="w-52 rounded-lg border border-border/60 bg-surfaceElevated px-3 py-2 text-sm text-foreground outline-none transition focus:border-accent/70" value={props.value.id} onInput={(event) => {
            const next = props.assistants.find((assistant) => assistant.id === event.currentTarget.value);
            if (next)
                props.onSelect(next);
        }}>
              {props.assistants.map((assistant) => (<option value={assistant.id}>{assistant.name}</option>))}
            </select>
          </Show>
