...
const buildInitialMessage = (assistant: AssistantPreset): ChatMessage => ({
...
const ChatPanel = (props: ChatPanelProps): JSX.Element => {
  const [messages, setMessages] = createSignal<ChatMessage[]>([buildInitialMessage(props.assistant)]);
  ...
  const simulateAssistantResponse = (messageId: string, prompt: string) => {
    const blueprints = [
  ...
  return (
    <section class="flex min-w-0 flex-1 flex-col gap-4 overflow-hidden rounded-2xl border border-border/70 bg-surface/70 p-5 shadow-glow">
      ...
      <div class="flex-1 overflow-y-auto rounded-xl border border-border/60 bg-surface/80 p-4">
        <For each={messages()}>
          {(message) => (
            <article
              class={`mb-4 flex gap-3 rounded-xl border border-transparent p-4 transition ${
                message.role === "assistant" ? "bg-surfaceElevated/60" : "bg-surface"
              }`}
            >
              <div
                class={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-border/80 text-sm font-semibold ${
                  message.role === "assistant" ? "bg-accent/15 text-accent-foreground" : "bg-surface/80 text-muted"
                }`}
              >
              ...
              <div class="flex flex-col gap-2">
                ...
                <Show when={message.highlights}>
                  <ul class="mt-2 space-y-1 text-xs text-muted">
                    <For each={message.highlights}>
                      {(item) => (
                        <li class="flex items-center gap-2 rounded-lg border border-border/50 bg-surface/60 px-3 py-1.5">
                ...
                <div class="flex gap-2 text-xs text-muted">
                  <button
                    class="rounded-lg border border-border/70 bg-surface px-3 py-1 transition hover:border-accent/60 hover:text-accent-foreground"
                    onClick={() => props.onInsertCode("// Apply patch\n" + message.content)}
                  >
                    Insert into editor
                  </button>
                  <button
      ...
      <div class="rounded-2xl border border-border/60 bg-surface/80 p-4">
        ...
        <textarea
          class="mt-3 h-28 w-full resize-none rounded-xl border border-border/60 bg-codeBg px-4 py-3 text-sm text-foreground outline-none transition focus:border-accent/70"
          placeholder="Ask Claude to review the Solid state management or generate feature scaffolding..."
          value={input()}
        ...
        <div class="mt-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div class="flex items-center gap-2">
            <button
              class="rounded-xl border border-border/60 bg-surface px-3 py-2 text-xs text-muted transition hover:border-accent/60 hover:text-accent-foreground"
              onClick={() => {
                setInput("");
            ...
            <button
              class="rounded-xl bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground transition hover:bg-accent/85"
              onClick={sendMessage}
            >
              Send ↵
            </button>
